...
extern unsigned char Key[32];		// clé "parisestmagique!"
extern unsigned char RoundKey[240]; // les différentes sous-clés
// extern unsigned int graine; //la graine initiale pour la fonction chaotique
...
void ShiftRows2() // Bien comprendre avant d'inverser la fonction
{
	unsigned char temp;

	temp = state[0][0] >> 7;
	state[0][0] <<= 1; // state[0][0] = state[0][0] << 1
	state[0][0] ^= (state[0][1] >> 7);
	state[0][1] <<= 1;
	state[0][1] ^= (state[0][2] >> 7);
	state[0][2] <<= 1;
	state[0][2] ^= (state[0][3] >> 7);
	state[0][3] <<= 1;
	state[0][3] ^= temp;

	temp = state[1][0] >> 6;
	state[1][0] <<= 2;
	state[1][0] ^= (state[1][1] >> 6);
	state[1][1] <<= 2;
	state[1][1] ^= (state[1][2] >> 6);
	state[1][2] <<= 2;
	state[1][2] ^= (state[1][3] >> 6);
	state[1][3] <<= 2;
	state[1][3] ^= temp;

	temp = state[2][0] >> 5;
	state[2][0] <<= 3;
	state[2][0] ^= (state[2][1] >> 5);
	state[2][1] <<= 3;
	state[2][1] ^= (state[2][2] >> 5);
	state[2][2] <<= 3;
	state[2][2] ^= (state[2][3] >> 5);
	state[2][3] <<= 3;
	state[2][3] ^= temp;

	temp = state[3][0] >> 4;
	state[3][0] <<= 4;
	state[3][0] ^= (state[3][1] >> 4);
	state[3][1] <<= 4;
	state[3][1] ^= (state[3][2] >> 4);
	state[3][2] <<= 4;
	state[3][2] ^= (state[3][3] >> 4);
	state[3][3] <<= 4;
	state[3][3] ^= temp;
}
...
void Cipher_Mode_1(FILE *FIC_IN, FILE *FIC_OUT)
{
	...
	{
		k = 0;
		while (k < 16 && (caract = fgetc(FIC_IN)) != EOF) /* chargement premier bloc */
		{
			in[k] = (unsigned char)caract;
			k++;
		}

		if (k == 0 && caract == EOF) /* condition de sortie  */
		{
			break;
		}
		else if (k > 0 && caract == EOF) /* dernier traitement avant sortie */
		{
			while (k < 16)
			{
				in[k] = 0;
...
{
	int i, k, caract, continuer = 1;
	unsigned char VI[16] = {0x87, 0x51, 0x1B, 0xC9, 0x95, 0xFB, 0x13, 0xA4, 0x0B, 0xE3, 0x4E, 0xC4, 0x33, 0xD5, 0x3D, 0xB3}; /* Vecteur d'initialisation */
	for (i = 0; i < 16; i++)																								 /* out prend le vecteur d'initialisation */
	{
		out[i] = VI[i];
	}
	while (continuer == 1)
	{
		...
		while (k < 16 && (caract = fgetc(FIC_IN)) != EOF) /* Chargement premier bloc */
		{
			in[k] = (unsigned char)caract; /* in texte en clair */
			k++;
		}

		if (k == 0 && caract == EOF) /* Condition d'arret */
		{
			break;
		}
		else if (k > 0 && caract == EOF) /* Dernier traitement avant arret */
		{
			while (k < 16)
			{
				in[k] = 0;
		...
		for (i = 0; i < 16; i++) /* Xor clair avec vecteur */
		{
			in[i] = in[i] ^ out[i];
		}
		...
		for (i = 0; i < Nb * 4; i++) /* Ecriture dans le fichier de sortie */
		{
			fputc(out[i], FIC_OUT);
		}
	}
...
void InvCipher_Mode_2(FILE *FIC_IN, FILE *FIC_OUT)
{
	int i, k, caract, continuer = 1;
	unsigned char VI[16] = {0x87, 0x51, 0x1B, 0xC9, 0x95, 0xFB, 0x13, 0xA4, 0x0B, 0xE3, 0x4E, 0xC4, 0x33, 0xD5, 0x3D, 0xB3}; /* Vecteur d'initialisation */
	unsigned char VItemp[16];

	k = 0;
	while (k < 16 && (caract = fgetc(FIC_IN)) != EOF) /* Chargement premier bloc */
	{
		in[k] = (unsigned char)caract;	   /* in texte chiffre */
		VItemp[k] = (unsigned char)caract; /* in texte chiffre */
		k++;
	}

	InvCipher();

	for (i = 0; i < 16; i++) /* Xor chiffre avec vecteur */
	{
		out[i] = out[i] ^ VI[i];
	}
	...
	while (continuer == 1)
	{
		...
		for (i = 0; i < 16; i++) /* Xor chiffre avec vecteur */
		{
			out[i] = out[i] ^ VItemp[i];
			VItemp[i] = in[i]; /* in texte chiffre */
		}
