...
double indice_coincidence(int *texte, int longueur_texte)
{
  ...
  for (int i = 0; i < 26; ++i)
    ind += ((compter_lettre(texte, (double)longueur_texte, i) / (double)longueur_texte) * (compter_lettre(texte, (double)longueur_texte, i) / (double)longueur_texte));

  return ind;
}

/****       Fonction pour extraire un sous-texte                         *******/
/*On tient compte de la période et du décalage à effectuer à chaque fois pour bien se positionner. En fait periode désigne la taille de la clé*/
void extraction_sous_texte(int *texte, int *sous_texte, int longueur_sous_texte, int periode, int indice_dec)
{
  for (int i = 0; i < longueur_sous_texte; ++i)
    *(sous_texte + i) = *(texte + periode * i + indice_dec);
}
